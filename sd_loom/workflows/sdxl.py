...
def _encode_prompt(pipe: Any, spec: SpecProtocol) -> dict[str, Any]:
    ...
    from compel import Compel, ReturnedEmbeddingsType  # type: ignore[import-untyped]
    ...
    try:
        ...
        prompt_embeds, pooled = compel(spec.prompt.positive)  # type: ignore[misc]
        neg_embeds, neg_pooled = compel(spec.prompt.negative or "")  # type: ignore[misc]
    finally:
