"""SDXL txt2img with k-diffusion sampling (A1111/ComfyUI-compatible)."""
...
def run(spec: SpecProtocol) -> list[GenerationResult]:
    """SDXL txt2img with k-diffusion sampling."""
    ...
    # --- Encode prompts with compel (text encoders to CUDA, encode, back) ---
    ...
    denoiser: Any = K.external.CompVisDenoiser(wrapper, quantize=False)
    ...
    for seed in seeds:
        click.echo(
            ...
            f"scheduler {spec.scheduler} (k-diffusion)"
        )
